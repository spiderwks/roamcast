...
import {
  ...
  Mountain, Footprints, Bike, Waves, Ship, Car,
} from 'lucide-react'
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        />
        <div className="text-right text-[9px] text-text-disabled mt-1">
          ...
          {ADVENTURE_TYPES.map(({ id, label, Icon }) => {
            ...
            return (
              <button
                key={id}
                type="button"
                onClick={() => setForm(f => ({ ...f, adventure_type: id }))}
                className={`flex items-center gap-3 p-3 rounded-sm border transition-colors ${
                  selected
                    ? 'border-brand-teal bg-brand-teal-deeper'
              ...
              >
                ...
                <span className={`text-[11px] font-medium ${selected ? 'text-white' : 'text-text-secondary'}`}>
                  {label}
                </span>
              </button>
            )
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            className="w-full bg-surface border border-border rounded-sm px-3 py-2.5 text-[12px] font-bold text-[#ccc] focus:border-brand-teal transition-colors"
          />
        </div>
        <div>
          <label className="block text-[9px] uppercase tracking-widest text-text-muted mb-1.5">
      ...
      <div>
        ...
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={addFollower}
            className="bg-brand-teal-deeper border border-brand-teal rounded-sm px-3 flex items-center justify-center"
          >
  ...
  return (
    <div className="space-y-5">
      <div className="bg-surface border border-border rounded-lg p-4">
        <p className="text-[13px] font-medium text-white mb-1">Trip summary</p>
      ...
      <p className="text-[11px] text-text-disabled text-center leading-relaxed">
        You can update all settings after creating the trip.
      </p>
    </div>
  )
}
    ...
    try {
      const { data: trip, error: tripErr } = await supabase
        ...
        .insert({
          ...
          adventure_type: form.adventure_type,
  ...
  return (
    <div className="flex flex-col h-full bg-surface-deep">
      ...
      <div className="px-4 pb-6 pt-4">
        ...
        >
          {saving ? 'Creating trip…' : step < STEPS.length - 1 ? (
        ...
        {step === 0 && (
          <p className="text-center text-[10px] text-text-disabled font-light mt-2">
            Followers receive an email invite when you start each day
          </p>
        )}
      </div>
    </div>
